...
test('runBench measures load, single-stream, and concurrent aggregate rates', async () => {
  ...
  // 1 warm-up call + 3 single-stream runs + 4 concurrent calls.
...
test('runBench warms up first and excludes the warm-up call from timing', async () => {
  ...
  const client = sequencedClient({ sequence: [1_000, 64, 64, 64, 64, 64, 64], parallel: 4 });
  const result = await runBench({
    endpoint,
    model: 'bench-model',
    client,
  ...
  // 1 warm-up + 2 single runs + 4 concurrent = 7 calls; the warm-up is first.
...
test('runBench reports the mean of the configured single-stream runs', async () => {
  // Warm-up 64, then single runs of 64 and 32 tokens (1s each) -> mean 48.
  const client = sequencedClient({ sequence: [64, 64, 32, 64, 64, 64, 64], parallel: 4 });
  const result = await runBench({
    endpoint,
    model: 'bench-model',
    client,
...
test('runBench flags a still-inverted aggregate as unreliable after one retry', async () => {
  ...
  const client = sequencedClient({
    sequence: [64, 64, 8, 8, 8, 8, 64, 8, 8, 8, 8],
    parallel: 4,
  });
  const ticks = [0, 1_000, 2_000, 3_000, 4_000, 5_000, 6_000, 7_000, 8_000, 9_000];
  let index = 0;
  ...
  // 1 warm-up + (1 single + 4 concurrent) x 2 attempts.
...
test('runBench recovers without a warning when the retry un-inverts', async () => {
  const client = sequencedClient({
    sequence: [64, 64, 8, 8, 8, 8, 64, 64, 64, 64, 64],
    parallel: 4,
  });
  const ticks = [0, 1_000, 2_000, 3_000, 4_000, 5_000, 6_000, 7_000, 8_000, 9_000];
  let index = 0;
...
test('runBench aggregates concurrently rather than multiplying single rate by slots', async () => {
  ...
  const client = {
    async chat() {
      call += 1;
      // Warm-up and the single-stream run return 64 tokens; the concurrent
      // phase returns fewer tokens per stream.
      const usage = { completion_tokens: call <= 2 ? 64 : 16 };
  ...
  // Clock: load=1s, single run=2s, concurrent=1s wall.
  const ticks = [0, 1_000, 2_000, 4_000, 5_000, 6_000];
  let index = 0;
  const nowFn = () => ticks[index++];
