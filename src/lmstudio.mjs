...
async function requestJson(endpoint, pathname, init = {}, options = {}) {
  requireEndpoint(endpoint);
  ...
  try {
    ...
    throw new Error(`LM Studio request to endpoint "${endpoint.id}" failed: ${error.message}`, {
      cause: error,
    });
  }
  ...
  if (!response.ok) {
    throw new Error(
      `LM Studio endpoint "${endpoint.id}" returned HTTP ${response.status}: ${body}`,
    );
  }
  try {
    ...
    throw new Error(
      `LM Studio endpoint "${endpoint.id}" returned invalid JSON: ${error.message}`,
      { cause: error },
    );
  }
}
...
export async function chat(
  ...
  if (!message) {
    throw new Error(`LM Studio endpoint "${endpoint.id}" returned no chat message`);
  }
  return {
    message,
